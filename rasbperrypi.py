...
# ─── Fare / klavye kontrolü (pynput — X11 gerektirir) ────────────────────────
try:
    ...
    _PYNPUT = False
    print("[UYARI] pynput bulunamadi: pip install pynput")
...
# ─── Ekran çözünürlüğü (Windows API yok — xrandr/tkinter ile tespit) ─────────
def _screen_size():
    try:
...
print(f"[EKRAN] {SCREEN_W}x{SCREEN_H}")
...
# ─── Tuş sabit değerleri (Linux karşılıkları) ────────────────────────────────
VK_BACK   = 'backspace'
VK_RETURN = 'enter'

# ─── Klavye / fare gönderici (pynput) ────────────────────────────────────────
def send_char(ch):
    if _PYNPUT:
        _kbd.type(ch)

def send_vk(key_name):
    if not _PYNPUT: return
    k = {VK_BACK: _Key.backspace, VK_RETURN: _Key.enter}.get(key_name)
    if k:
...
def left_click():
    if _PYNPUT:
        _mouse.press(_MB.left); _mouse.release(_MB.left)
...
def _release_camera(cap):
    if _PICAM and isinstance(cap, Picamera2):
        try: cap.stop()
...
def process_frame(cap, fl):
    ...
    h, w = frame.shape[:2]
    sc = PROCESS_WIDTH/w if w > PROCESS_WIDTH else 1.
    small = cv2.resize(frame,(int(w*sc),int(h*sc))) if sc<1. else frame
    # picamera2 BGR888 → BGR; OpenCV de BGR → her ikisinde de BGR2RGB gerekli
...
#
#  Çift kırpma mantığı:
#    İlk kırpma biter → pencere başlar, henüz 'advance' gönderilmez
#    Pencere içinde ikinci kırpma → 'double' gönderilir
#    Pencere geçer, ikinci kırpma gelmezse → 'advance' gönderilir
#    Göz KAPALI iken pending timeout asla ateşlenmez.
# ─────────────────────────────────────────────────────────────────────────────
            ...
            for ci, key in enumerate(items):
                ...
                if is_cur_row and not is_cur:
                    cv2.rectangle(canvas, (x0,y0), (x1,y1),
                                  (60,120,60) if kind=='key' else (60,90,140), 1)
...
def main():
    ...
    if cal is None: _release_camera(cap); fl.close(); return
    gm.calibrate(*cal)
    ...
    while True:
        ...
        if result.face_landmarks and result.facial_transformation_matrixes:
            ...
            if not kb_mode and gaze_pos and mouse_active:
                move_mouse(gaze_pos[0], gaze_pos[1])
