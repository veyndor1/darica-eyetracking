...
class GazeMapper:
    def __init__(self):
        ...
        self._last_good_irx=None;self._last_good_iry=None
    def calibrate(self,fl,sp):
        ...
        self._last_blink_t=0.0; self._last_good_irx=None; self._last_good_iry=None
    def map(self,features,blinking=False):
        if self.Mx is None or features is None: return self.kalman.predict_only()
        now=time.perf_counter()
        yaw0,pitch0,irx0,iry0,vg0=features
        # Kırpma sırasında iris göz kapağı tarafından bozulur — son iyi iris değerini besle.
        blink_or_freeze=blinking or (now-self._last_blink_t<0.10)
        if blink_or_freeze and self._last_good_irx is not None:
            irx_feed=self._last_good_irx; iry_feed=self._last_good_iry
        else:
            irx_feed=irx0; iry_feed=iry0
        yaw=self._ow.update(yaw0,now); pitch=self._op.update(pitch0,now)
        irx=self._ox.update(irx_feed,now); iry=self._oy.update(iry_feed,now); vg=self._ov.update(vg0,now)
        if not blink_or_freeze:
            self._last_good_irx=irx0; self._last_good_iry=iry0
        if blinking:
            self._last_blink_t=now; return self.sent_pos
        if now-self._last_blink_t<0.10: return self.sent_pos
        if self._py is not None:
            if abs(yaw0-self._py)>HEAD_DELTA_GATE or abs(pitch0-self._pp)>HEAD_DELTA_GATE:
                self._py,self._pp=yaw0,pitch0; return self.sent_pos
        self._py,self._pp=yaw0,pitch0
        if self.tps:
...
class ScanBlinkDetector:
    ...
    def update(self, ear: float, now: float, iry: float = 0.0):
        ...
        # ── Pending single blink zamanaşımı (her karede kontrol) ─────
        ...
        if closed:
            ...
            if not self._fired and held >= self.TOGGLE_S:
                self._fired   = True
                self._pending = False    # olası pending'i iptal et
                return 'toggle', 1.0

            prog = min(held / self.ROW_SKIP_S, 1.0)
            # Pending zamanaşımı ile toggle aynı karede üst üste gelirse
            # toggle önceliklidir (yukarıda return etti); buraya geldiyse toggle yok.
            if pending_advance:
                return 'advance', prog
            return None, prog
            ...
            # ── GÖZ AÇIK ─────────────────────────────────────────────
            # Taban çizgisini yavaşça güncelle (sadece göz açıkken)
            self._base = 0.97 * self._base + 0.03 * ear
            self._pe   = ear
            ...
            if self._t0 is not None:
                ...
                # Geçerli bir kırpma mı? (toggle ateşlenmemişse + minimum kriterler)
                if not was_toggled and prev_frames >= self.MIN_FRAMES and held >= self.NOISE_S:

                    if held >= self.ROW_SKIP_S:
                        # Orta tutma → satır atla
                        self._pending = False
                        return 'row_skip', 0.0

                    else:
                        # Kısa kırpma — tek mi çift mi?
                        if self._pending:
                            # İkinci kırpma pencere içinde → ÇİFT KIRPMA → SEÇ
                            self._pending = False
                            return 'double', 0.0
                        else:
                            # İlk kırpma → pending başlat, henüz advance gönderme
                            self._pending   = True
                            self._pending_t = now

            # Pending zamanaşımı bu karede tetiklendiyse
            if pending_advance:
                return 'advance', 0.0
            return None, 0.0
