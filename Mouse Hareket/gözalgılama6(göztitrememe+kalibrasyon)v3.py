...
GAZE_Y_FLIP          = -1   # Y ekseni ters geliyorsa -1 bırak, düzelirse 1 yap
...
def compute_gaze_features(lms, transform_matrix_data):
    """
    4 özellikli gaze vektörü döner: [head_yaw, head_pitch, iris_rel_x, iris_rel_y]

    X ← [yaw, iris_rel_x, 1]  : göz genişliğine normalize yatay ofset
    Y ← [pitch, iris_rel_y, 1] : göz yüksekliğine normalize dikey ofset
    Her iki eksende de relatif konum kullanılıyor: kafa hareket edince göz sınırları da
    birlikte kayar, relatif değer sabit kalır → pitch/iris_rel_y bağımsız → multicollinearity yok.
    """
    ...
    return [yaw, pitch, iris_rel_x, iris_rel_y * GAZE_Y_FLIP]
...
# ─────────────────────────────────────────────
# Polinom özellik fonksiyonları — derece-2
# ─────────────────────────────────────────────
# Doğrusal model: X = w0*yaw + w1*iris_x + w2
# Polinom model:  X = w0*yaw + w1*iris_x + w2*(yaw*iris_x) + w3*iris_x² + w4*yaw² + w5
# Göz geometrisi küresel: köşelerde sapma polinom terimleriyle düzeltilir.

def _feat_x(yaw, iris_rel_x):
    # Lineer model — extrapolasyon köşelerde tahmin bükülmez
    return np.array([yaw, iris_rel_x, 1.0], dtype=float)

def _feat_y(pitch, iris_rel_y):
    return np.array([pitch, iris_rel_y, 1.0], dtype=float)
...
class GazeMapper:
    """
    X ve Y eksenleri tamamen ayrı özellik setleriyle map edilir.

    X ← [yaw, iris_rel_x, 1]   : yatay sinyaller → yatay konum
    Y ← [pitch, iris_abs_y, 1] : dikey sinyaller  → dikey konum

    Böylece sola/sağa bakış Y'yi, yukarı/aşağı bakış X'i hiç etkilemez.
    """
    def __init__(self):
        self.Mx        = None   # şekil (3,): [yaw, iris_rel_x, 1] → screen_x
        self.My        = None   # şekil (3,): [pitch, iris_rel_y, 1] → screen_y
    ...
    def calibrate(self, features_list, screen_pts):
        # features_list elemanı: [yaw, pitch, iris_rel_x, iris_rel_y]
        ...
        #   kullanıcı %8 noktasına bakarken cursor 0'a, %92'ye bakarken W'ya gider.
        # Böylece ekran dışına çıkmadan köşelere ulaşılır.
        pred_x = Ax @ self.Mx
        pred_y = Ay @ self.My
    ...
    def map(self, features, blinking=False):
        ...
        yaw, pitch, iris_rel_x, iris_rel_y = features
