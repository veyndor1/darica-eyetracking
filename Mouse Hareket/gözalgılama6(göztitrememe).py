...
DEAD_ZONE            = 7    # piksel — bu kadardan küçük hareketler yok sayılır
DOUBLE_BLINK_MAX_GAP = 1.5  # iki kırpma arası max süre (s) — 1.5s pencere
GAZE_X_OFFSET        = 0    # negatif = imleç sola kayar; sağa kayma varsa negatif yap (örn. -40)
...
class GazeMapper:
    ...
    def calibrate(self, features_list, screen_pts):
        ...
        Ax = np.array([[f[0], f[2], 1.0] for f in features_list], dtype=float)  # [yaw, iris_rel_x, 1]
        Ay = np.array([[f[1], f[3], 1.0] for f in features_list], dtype=float)  # [pitch, iris_rel_y, 1]
    ...
    def map(self, features, blinking=False):
        ...
        sx = float(np.clip(np.array([yaw, iris_rel_x, 1.0]) @ self.Mx + GAZE_X_OFFSET, 0, SCREEN_W - 1))
        sy = float(np.clip(np.array([pitch, iris_rel_y, 1.0]) @ self.My + GAZE_Y_OFFSET, 0, SCREEN_H - 1))
