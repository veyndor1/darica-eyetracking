...
GAZE_Y_OFFSET        = 50   # pozitif = imleç aşağı kayar (baktığın yere göre düzeltme)
...
def compute_gaze_features(lms, transform_matrix_data):
    """
    4 özellikli gaze vektörü döner: [head_yaw, head_pitch, iris_rel_x, iris_abs_y]

    - iris_rel_x : iris'in göz genişliğine göre yatay ofseti (yatay ince ayar)
    - iris_abs_y : iris'in FRAME içindeki mutlak dikey konumu (0=üst, 1=alt)

    iris_abs_y neden daha iyi:
      Göz içi dikey ofset çok küçük (±0.003), normalize edince gürültüye gömülür.
      Mutlak Y ise 0.35–0.55 aralığında seyreder, kalibrasyon için güçlü sinyal verir.
      Hem baş eğimini hem göz hareketini birlikte yansıtır.
    """
    yaw, pitch = get_head_angles(transform_matrix_data)

    # Yatay: iris ofseti göz genişliğine normalize (çalışıyor, değiştirme)
    ...
    # Dikey: iris'in frame içindeki mutlak Y konumu
    iris_abs_y = (lms[468].y + lms[473].y) / 2.0

    return [yaw, pitch, iris_rel_x, iris_abs_y]
...
class GazeMapper:
    ...
    def __init__(self):
        ...
        self.My       = None   # şekil (3,): [pitch, iris_abs_y, 1] → screen_y
        self.kalman   = KalmanGaze()
        self.sent_pos = (SCREEN_W // 2, SCREEN_H // 2)

    def calibrate(self, features_list, screen_pts):
        # features_list elemanı: [yaw, pitch, iris_rel_x, iris_abs_y]
        Ax = np.array([[f[0], f[2], 1.0] for f in features_list], dtype=float)
        Ay = np.array([[f[1], f[3], 1.0] for f in features_list], dtype=float)
    ...
    def map(self, features, blinking=False):
        ...
        yaw, pitch, iris_rel_x, iris_abs_y = features
        sx = float(np.clip(np.array([yaw, iris_rel_x,  1.0]) @ self.Mx, 0, SCREEN_W - 1))
        sy = float(np.clip(np.array([pitch, iris_abs_y, 1.0]) @ self.My + GAZE_Y_OFFSET, 0, SCREEN_H - 1))
