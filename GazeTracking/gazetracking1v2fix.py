...
CAL_WAIT=1.2; CAL_COLLECT=1.8; CAL_RIDGE=0.02; EAR_BLINK=0.20
DEAD_ZONE=0; GAZE_X_OFFSET=0; GAZE_Y_OFFSET=-2; GAZE_Y_FLIP=1
HEAD_DELTA_GATE=0.22; TPS_IRIS_WEIGHT=1.5
# Çıkış genişletme — TPS/polinom çıktısını ekran merkezi etrafında çarpar.
# Y_EXPAND > 1: kalibrasyondaki dikey aralık ekrana daha çok yayılır
# (yukarıya bakınca tam üste ulaşır, ortada kalan +30/-35 kayma giderilir).
CAL_EXPAND_X = 1.0
CAL_EXPAND_Y = 1.15
# Titreme azaltma — One Euro filtreye dokunmadan Kalman ve ölü bölge ile düşürülür.
...
class GazeMapper:
    ...
    def map(self,features,blinking=False):
        ...
        # Ekran merkezi etrafında genişletme — dikey/yatay erişimi düzeltir.
...
def _is_eye_closed(ear, baseline, prev_ear, in_blink):
    """
    Adaptif kırpma kontrolü.
    - Başlangıç: hem baseline*0.78 altı hem de ani düşüş (>0.025) gerekli.
    - Devam: zaten kırpma sürecindeyse sadece eşik altı yeterli.
    Aşağı bakış EAR'ı yavaş düşürür → ani düşüş kriteri yanlış pozitifi önler.
    """
...
class BlinkDetector:
    ...
    def update(self,ear,now):
        ...
        db=False
        closed = _is_eye_closed(ear, self._base, self._pe, self.in_b)
        if closed:
            self.frames+=1; isb=self.frames>=2
        else:
            ...
            # Göz açıkken taban çizgisini yavaş güncelle
...
class ScanBlinkDetector:
    ...
    def __init__(self):
        ...
        self._base = None     # adaptif EAR taban çizgisi
        self._pe = None       # bir önceki frame EAR değeri
    ...
    def update(self, ear: float, now: float):
        ...
        Adaptif eşik + ani düşüş kontrolü ile aşağı bakışta yanlış tetiklemeyi
        engeller.
        """
        if self._base is None: self._base = ear; self._pe = ear

        in_blink = self._t0 is not None
        closed = _is_eye_closed(ear, self._base, self._pe, in_blink)

        if closed:                                    # ── gözler kapalı ──
            if self._t0 is None:
            ...
            self._base = 0.97*self._base + 0.03*ear   # taban çizgisini güncelle
            self._pe = ear
            if self._t0 is None:
                return None, 0.0
            held = now - self._t0
